...
/*! \file gmpbiplane.c
 *
 *  Implementation of the PBiPlane template class.
 */
...
#include "gmpbiplane.h"
  ...
  template <typename T>
  inline
  PBiPlane<T>::PBiPlane( const GMlib::Point<T,3>& p1, const GMlib::Point<T,3>& p2, const GMlib::Point<T,3>& p3, const GMlib::Point<T,3>& p4 ) {
  ...
  template <typename T>
  inline
  PBiPlane<T>::PBiPlane( const PBiPlane<T>& copy ) : GMlib::PSurf<T,3>( copy ) {
  ...
  template <typename T>
  void PBiPlane<T>::eval(T u, T v, int d1, int d2, bool /*lu*/, bool /*lv*/ ) {
  ...
  template <typename T>
  T PBiPlane<T>::getEndPU()	{

    return T(1);
  }

  template <typename T>
  T PBiPlane<T>::getEndPV()	{
